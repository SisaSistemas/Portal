...
namespace SisaDev.Administracion
{
    public partial class ProyectoDetallesAdmin : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                SisaEntitie sisaEntie = new SisaEntitie();
                ...
                if (usuario != null)
                {
                    ...
                    using (sisaEntie)
                    {
                        tblProyectos tblProyectos = sisaEntie.tblProyectos.First<tblProyectos>((Expression<Func<tblProyectos, bool>>)(p => p.IdProyecto.ToString() == ProyectoDetallesAdmin.idProyecto));
        ...
        public static string CargarDatos(string IdProyecto, string Opcion)
        {
            ...
            try
            {
                using (SisaEntitie sisaEntitie = new SisaEntitie())
                    return JsonConvert.SerializeObject((object)sisaEntitie.sp_CargarDetalleProyecto(IdProyecto, Convert.ToInt32(Opcion)));
            }
            catch (Exception ex)
            {
                return ex.ToString();
        ...
        public static string CargarComentarios(string IdProyecto, string Opcion)
        {
            ...
            {
                using (SisaEntitie sisaEntitie = new SisaEntitie())
                    return JsonConvert.SerializeObject(sisaEntitie.sp_CargarDetalleProyecto(IdProyecto,Convert.ToInt32(Opcion)));
            }
            catch (Exception ex)
            {
                return ex.ToString();
        ...
        [WebMethod]
        public static string CargarDetalleGrafica(string Nombre, string IdProyecto, decimal Gastado)
        {
            
            try
            {
                using (SisaEntitie sisaEntitie = new SisaEntitie())
                    return JsonConvert.SerializeObject(sisaEntitie.sp_GraficaPastelDetalle(IdProyecto, Nombre, Convert.ToDecimal(Gastado)));
            }
            catch (Exception ex)
            {
                return ex.ToString();
